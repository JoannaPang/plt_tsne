...
import numpy as np
load_fn = 'lotsoffeatures/dqntemporalduke1218ac11218temporaldense121duke_pytorch_result.mat'
...
top15_list=[]
top15_list_value=[]
tempy=np.bincount(y)#把数组中出现的每个数字，当作index，数字出现的次数当作value来表示
#print(tempy)
for i in range(15):
...
for i in range(len(top15_list)):
    ...
    small_index=small_index+kkkkk2[0] #将list数组进行连接
    #print(small_index)
...
#print(type(qu10))
X_small=X[small_index,:] #取numpy数组中的前500行元素
y_small=y[small_index]#取numpy数组中的前500个元素
#print(X_small.shape)
#print(y_small.shape)
#print(y_small)
