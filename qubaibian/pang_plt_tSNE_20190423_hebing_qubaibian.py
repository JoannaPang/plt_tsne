...
import numpy as np
import copy
#load_fn = 'lotsoffeatures/dqntemporalmarket1218ac11218temporaldense121vlids_pytorch_result.mat'

load_fn = '99prwdensnentcoco_dense121rgbmask_prw_densenet_mbp_pytorch_result.mat'
...
X_gallery = load_data['gallery_f'] #X为特征
y_gallery = load_data['gallery_label'] #y为label，即文件夹名，即pid

'''
print('X_gallery shape')
print(X_gallery.shape) #查看numpy.ndarray的形状用X.shape
print('y_gallery shape')
print(y_gallery.shape)
'''

X_query = load_data['query_f']

y_query = load_data['query_label']
...
X_trainall = load_data['train_all_f']

y_trainall = load_data['train_all_label']
...
#gallery, query, train_all hebing
X = np.vstack((X_gallery, X_query, X_trainall)) #将元组中的指定数组进行合并
y_temp = np.append(y_gallery, y_query) #直接合并成一个长数组，参数只能为两个
y = np.append(y_temp, y_trainall)


#test
#X = np.append(X_gallery, X_query, axis=0)

#y = np.append(y_gallery, y_query, axis=0)

#X = np.concatenate((X_gallery, X_query), axis=0)

#y = np.concatenate((y_gallery, y_query), axis=0)
...
'''
print(type(X)) #X的类型为numpy.ndarray
print(type(y)) #y的类型为numpy.ndarray
print('x_dim')
print(X.ndim)
print('y_dim')
print(y.ndim)
...
print('load_success!')
small_index = []
for i in range(2, len(top15_list)):
    #print(top15_list[i])
...
for i, label in zip(target_ids, labels):
    ...
    if int(label)//10 == 0:
        ...
        plt.scatter(X_2d[y_small == i, 0], X_2d[y_small == i, 1], label=label,
                    marker='v', edgecolors='black', linewidths=0.7, alpha=0.8)
    elif int(label)//10 == 6:
        plt.scatter(X_2d[y_small == i, 0], X_2d[y_small == i, 1], label=label,
                    marker='*', edgecolors='black', linewidths=0.7, alpha=0.8)
    elif int(label)//10 == 7:
        plt.scatter(X_2d[y_small == i, 0], X_2d[y_small == i, 1], label=label,
                    marker='p', edgecolors='black', linewidths=0.7, alpha=0.8)
    elif int(label)//10 == 8:
        plt.scatter(X_2d[y_small == i, 0], X_2d[y_small == i, 1], label=label,
                    marker='+', edgecolors='black', linewidths=0.7, alpha=0.8)
    elif int(label)//10 == 9:
        plt.scatter(X_2d[y_small == i, 0], X_2d[y_small == i, 1], label=label,
                    marker='h', edgecolors='black', linewidths=0.7, alpha=0.8)

#plt.legend()#图例和注记
#plt.rcParams['font.family']='Times New Roman'  #所有的字体都是新罗马 #注意该设置不能对刻度进行设置
#fontPath = '/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf'
#prop = font_manager.FontProperties(fname = '/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf')
'''
prop = font_manager.FontProperties(fname = '/usr/share/fonts/truetype/msttcorefonts/timesbd.ttf')
plt.rcParams['font.family']=prop.get_name()  #所有的字体都是新罗马
print(prop.get_name())
...
'''
plt.subplots_adjust(top=0.98,bottom=0.07,left=0.1,right=0.98,hspace=0,wspace=0) #设置存储时白边问题
plt.margins(0,0)

# 保存图片到指定路径
plt.savefig('hebing.pdf')
plt.show()
